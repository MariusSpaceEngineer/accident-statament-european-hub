...
class SignatureFragmentInstrumentedUITest {
    ...
    fun setup() {
        ...
        launchFragmentInContainer<AccidentStatementSignatureFragment>(themeResId = R.style.Theme_CrashKit) {
            AccidentStatementSignatureFragment().also { fragment ->
    ...
    @Test
    fun testSignatureUIElements() {
        // Check to see if the fields are displayed on the screen
        onView(withId(R.id.sp_statement_vehicle_a))
            .check(matches(isDisplayed()))
        onView(withId(R.id.btn_statement_vehicle_a_disagree))
            .check(matches(isDisplayed()))
        onView(withId(R.id.sp_statement_vehicle_b))
            .perform(scrollTo())
            .check(matches(isDisplayed()))
        onView(withId(R.id.btn_statement_vehicle_b_disagree))
            .check(matches(isDisplayed()))

        onView(withId(R.id.btn_statement_vehicle_a_disagree)).check(matches(ViewMatchers.isClickable()))
        onView(withId(R.id.btn_statement_vehicle_b_disagree)).check(matches(ViewMatchers.isClickable()))
    }
}